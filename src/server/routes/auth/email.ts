...
router.get("/verify", async (req, res) => {
    const codeParams = req.query.code as string;

    if (!codeParams) return res.status(400).json({ message: "Missing code from the URL!" });
    let user_id, email;

    try {
        const [code] = await db.codes.get(codeParams);
        if (!code) throw new Error("Missing code");

        const [user] = await db.users.by("id", code.user_id);
